...
import { getFirestore, collection, addDoc, getDocs, query, where, doc, updateDoc, Timestamp, getDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
async function uiDashboard() {
    ...
    mainContent.innerHTML = `
        ...
        <div class="px-2 space-y-4">
            <button onclick="router('clientes')" class="w-full bg-blue-600 text-white p-6 rounded-[2rem] font-black shadow-xl flex items-center justify-between">
                <span>GESTIONAR CLIENTES</span>
                <i class="fas fa-users text-xl"></i>
            </button>
            <button onclick="router('prestamos')" class="w-full bg-white border-2 p-6 rounded-[2rem] font-black text-gray-700 flex items-center justify-between shadow-sm">
...
window.verPerfil = async (id, nombre, telefono) => {
    ...
    document.getElementById('f-p-final').onsubmit = async (e) => {
        ...
        await addDoc(collection(db, "prestamos"), { clienteId: id, totalConInteres: total, saldoActual: total, valorCuota: total/c, modalidad: document.getElementById('p-mod').value, estado: "activo", cobradorId: auth.currentUser.uid, fecha: new Date() });
...
window.regAbono = async (pId, cId, cNom, cTel) => {
    ...
    if (!input.value || parseFloat(input.value) <= 0) return alert("Escribe un monto");
    ...
    } catch (e) { alert("Error al registrar abono"); }
};

// --- HISTORIAL (FIX DE CARGA INFINITA) ---
async function uiPrestamos() {
    ...
    try {
        // Quitamos orderBy temporalmente para evitar error de índice
...
function uiCobros() { alert("Modulo de Cobros proximamente"); }